...
RUN apk add \
    ...
    gtksourceview4 \
    gtk+3.0 vte3 py3-libxml2 spice-gtk gtk-vnc py3-cairo\
    autoconf libtool m4 automake libtool\
...
# RUN ls; cd /usr; ls; cd local; ls;
# Install virt-manager with script from developer
RUN cd virt-manager && git checkout v3.2.0 && ./setup.py configure --prefix=/usr/local && ./setup.py install --exec-prefix=/usr/local
